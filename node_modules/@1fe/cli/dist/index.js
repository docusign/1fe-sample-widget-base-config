...
var widgetsSchema = zod.z.object({
  basePrefix: zod.z.string(),
  configs: zod.z.array(widgetConfigSchema)
});
var browserslistConfigSchema = zod.z.object({
  buildTarget: zod.z.array(zod.z.string()),
  unsupportedBrowserScreen: zod.z.array(zod.z.string())
});
var platformConfigSchema = zod.z.object({
  devtools: devtoolsSchema.optional(),
  browserslistConfig: browserslistConfigSchema
});
var dynamicConfigSchema = zod.z.object({
...
var environmentSchema = zod.z.object({
  ...
  libraryVersions: zod.z.array(managedSchema),
...
var getLibraryVersions = lodash.memoize(async (environment) => {
  const { baseConfig } = await getConfig();
...
async function getExternalsLayer(environment) {
  ...
  const externals = libraryVersions.filter((lib) => lib.type === "external").map((lib) => ({
    [lib.id]: lib.name
...
var getDynamicConfigs = lodash.memoize(async (environment) => {
  const { baseConfig } = await getConfig();
  if (!baseConfig.environments[environment]) {
    throw new Error(
      `No base configuration found for environment "${environment}"`
...
async function getTargetLayer(environment) {
  ...
  const browsersListRules = (await getDynamicConfigs(environment)).platform.browserslistConfig.buildTarget.join();
