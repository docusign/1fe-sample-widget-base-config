...
var stripAnsiFn = require('strip-ansi');
...
var stripFinalNewline__default = /*#__PURE__*/_interopDefaultLegacy(stripFinalNewline);
var stripAnsiFn__default = /*#__PURE__*/_interopDefaultLegacy(stripAnsiFn);
...
function getDefaultNormalizer(_temp) {
  ...
  return text => {
    ...
    normalizedText = stripAnsi ? stripAnsiFn__default["default"](normalizedText) : normalizedText;
...
const killProc = (instance, signal) => new Promise((resolve, reject) => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
  treeKill__default["default"](instance.process.pid, signal, async err => {
    try {
      if (err) {
...
function prettyCLI(testInstance, maxLength) {
  ...
  return maxLength !== undefined && outStr.length > maxLength ? sliceAnsi__default["default"](outStr, 0, maxLength) : outStr;
}
...
async function render(command, args, opts) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
  const exec = childProcess__default["default"].spawn(command, args, {
    ...spawnOpts,
    cwd,
    shell: true
  });
  ...
  exec.stdout.on('data', result => {
    ...
    const resStr = stripFinalNewline__default["default"](result);
